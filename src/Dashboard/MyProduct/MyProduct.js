...
const MyProduct = () => {
    ...
    const { refetch, error, data: items = [] } = useQuery({
        ...
        queryFn: async () => {
            const res = await fetch(`https://vagetable-server.vercel.app/addProduct?email=${user?.email}`)
    ...
    return (
        <div>
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8'>
                {
                    items?.data?.map(item => <SingleProduct key={item._id} item={item}></SingleProduct>)
                }
            </div>
        </div>
    );
